...
class MLPReadout(nn.Module):
    ...
    Now supports custom initialization for the last layer's bias.
    """
    def __init__(
        self,
        in_features: int,
        ...
        activation: str = "silu",
        dropout_p: float = 0.0,
        bias_init: float = None  # <--- NEW ARGUMENT
    ):
        super().__init__()
        ...
        final_layer = nn.Linear(current_features, out_features, bias=True)
        
        # --- NEW LOGIC: Initialize Bias ---
        if bias_init is not None:
            # We assume out_features=1 implies a scalar bias, but this works for vector biases too
            # The prompt requested "shift / len of last layer".
            # If out_features > 1, this distributes the value.
            val = bias_init / out_features 
            nn.init.constant_(final_layer.bias, val)
